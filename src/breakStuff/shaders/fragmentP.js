...
void main() {
 ...
 vec2 rote = rotate2D(uv, PI + t);
 vec2 roteC = rotate2D(uv, PI - t);

 uv4 = rotate2D(uv4, PI + t);
        ...
        coswarp(color, 3.);
          ...
          color = mix(color, vec3(1.),  step(shape(uv4, 4. + sin(uTime), .8 * length(uv-.5)).r, .5));
      
           color = mix(color,vec3(0.),   step(shape(uv4, 4. + sin(uTime), 1.6 * length(uv-.5)).r, .5));

         

      if (color == vec3(0.)){

        color = vec3( step(cnoise(rote * 14. * cnoise(roteC * 6.)), .1) );
      }
