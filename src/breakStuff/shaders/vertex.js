...
void coswarp(inout vec3 trip, float warpsScale ){
  ...
  void main(){
    ...
    modelPosition.xyz = mix(modelPosition.xyz, modelPosition2.xyz, .5);
    ...
    gl_Position = projectionPosition;
     gl_PointSize = mix( 
      (180. * ((sin(uTime) + length(modelPosition2.xy) +1. ) *.5)  )  * 1. * 1.,
      (180. * ((cos(uTime) + length(modelPosition.yz) +1. ) *.5)  )  * 1. * .5,
      sin(uTime)
    );
