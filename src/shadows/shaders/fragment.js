...
void main() {
  ...
  vec2 uv2 = vec2(uv.x +( sin(uTime) * .02), uv.y +(cos(uTime) * .02));

  vec2 uv3 = vec2(uv.x + (cos(uTime) * .012), uv.y +(sin(uTime) * .012));
 ...
 color.r = smoothstep(shape(uv, 3., .4).r, .4, .7);

 color.g = smoothstep(shape(uv2, 4., .4).r, .4, .7);

 color.b = smoothstep(shape(uv3, 5., .4).r, .4, .7);
