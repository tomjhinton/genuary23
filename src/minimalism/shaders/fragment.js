...
void main() {
  ...
  float t2 = (uTime *.75) +length(uv);

  uv = fract(uv * 4. + (sin(t) * 2.));
uv = rotate2D(uv, PI * length(uv-.5) + t);
 ...
 color += step(
   mix(
   mix(shape(uv, 3., .3).r, shape(uv, 5., .3).r, sin(t2)),
   mix(shape(uv, 4., .3).r, shape(uv, 60., .3).r, sin(t2 * .6)),
   sin(t2 * .3)
   )
 ...
 color = mix(color, 1.-color,
  step(
  mix(
    mix(shape(uv, 3., .2).r, shape(uv, 5., .2).r, sin(t)),
    mix(shape(uv, 4., .2).r, shape(uv, 6., .2).r, sin(t * .6)),
    sin(t2 * .3)
    )
